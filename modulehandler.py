...
class modulehandler:
    ...
    def write_modules_output(self, filename="redhatscoring.py"):
        ...
        conditionals = self.conditional_list

        intro_comments = """#!/usr/bin/python
#Code generated by the Red Hat Scoring Engine for the CAST Tech Cybersecurity Invitational.
#Engine Created by Caleb Loomis and Dr. Don Davis.
#Modules written by CAST Tech Cyberpatriots
#Released under GPL"""


        meat_of_code = """
total_points = len(conditionals)
actual_points = 0
solved = list()
for item in conditionals:
    if (eval(item["conditional"])):
        actual_points = actual_points + 1
        solved.append(item['point_text'])
print("Points: " + str(actual_points) + " / " + str(total_points))

if (len(solved) > 0):
    print ("Items solved:")
    for item in solved: print (item)"""
        ...
        with open (filename, 'w+') as engine_file:
            ...
            writeln(intro_comments)
            writeln(import_line)
            writeln("conditionals = " + str(conditionals))
            writeln(meat_of_code)
